...
function generate_configuration(system::SRXsystem)
    ...
    r_traj = collect_trajectory(sub_trajectory(trajectory, r_idxs))
...
function collect_samples(initial::SXconfiguration, ensemble::MarginalEnsemble, num_samples::Int)
    ...
    for result_col ∈ eachcol(result)
        integrator = DiffEqBase.init(jprob, SSAStepper(), numsteps_hint=0)
        iter = SSAIter(integrator)
        cumulative_logpdf!(result_col, ensemble.x_dist, merge_trajectories(iter, initial.x_traj), ensemble.dtimes, params=ensemble.p)
    end
...
function propagate(conf::SXconfiguration, ensemble::MarginalEnsemble, u0, tspan::Tuple)
    ...
    iter = SSAIter(integrator)
    log_weight = logpdf(ensemble.x_dist, merge_trajectories(iter, conf.x_traj), params=ensemble.p)

    integrator.u, log_weight
end

function energy_difference(configuration::SXconfiguration, ensemble::MarginalEnsemble)
    -cumulative_logpdf(ensemble.x_dist, merge_trajectories(configuration.s_traj, configuration.x_traj), ensemble.dtimes, params=ensemble.p)
end
...
function energy_difference(configuration::SRXconfiguration, ensemble::ConditionalEnsemble)
    ...
    -cumulative_logpdf(ensemble.x_dist, traj, ensemble.dtimes, params=ensemble.p)
end 
