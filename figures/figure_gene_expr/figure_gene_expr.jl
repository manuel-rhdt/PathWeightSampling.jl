...
ddir = projectdir("figure_gene_expr", "data")
...
example_trajectories = DataFrame(load(joinpath(ddir, "example_trajectories.csv")))
...
titlefont = projectdir("fonts", "NotoSans-SemiBoldItalic.ttf")
...
fig = with_theme(fontsize_theme) do
    f = Figure(resolution = (246, 1.4 * 146), figure_padding = 1.5)

    axs = (Axis(f[1,1], title="input S", ylabel="copy number"), Axis(f[1, 2], title="output X", yticklabelsvisible=false, yticksvisible=false))
    tightlimits!.(axs)
    linkyaxes!(axs...)
    colors = ["#008F00", "#0096FF"]
    for (ax, g, color) in zip(axs, groupby(example_trajectories, :Comp), colors)
        lines!(ax, g.t, g.Mean, color=color)
        for i in 1:5
            lines!(ax, g.t, g[!, Symbol(:traj, i)], color = (color, 0.4), linewidth=0.5)
        end
        band!(ax, g.t, g.Mean .- g.Std, g.Mean .+ g.Std, color = (color, 0.4))
        ylims!(ax, 30, 70)
    end
    ...
    m_label = [rich(rich("M", font=:italic), " = $(g.M[1])") for g in groups]
    ...
    for (i, group) in enumerate(groupby(ti_data, :M, sort = true))
        ...
        push!(ti_label, rich(rich("M", font=:italic), " = $(group.M[1])"))
    ...
    legend = Legend(
        ...
        [group_color, ti_color, smc_color, approx_entry],
        [m_label, ti_label,
        [rich(rich("M", font=:italic), " = 128")], [rich("Duso, ", rich("et al.", font=:italic), " (2019)"), "Gaussian (LNA)"]],
...
save(projectdir("figure_gene_expr", "gene-expr-figure.svg"), fig, pt_per_unit = 1)
save(projectdir("figure_gene_expr", "gene-expr-figure.pdf"), fig, pt_per_unit = 1)
